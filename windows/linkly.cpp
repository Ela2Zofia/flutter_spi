    ...
    do {
        ...
        if (iResult > 0) {
            ...
            if (curr_len == 0) {
                ...
                message_len = std::stoi(std::string(msg_length.begin(), msg_length.end()), nullptr, 10);
            }
            response.insert(response.end(), buffer_vec.begin(), buffer_vec.end());
            curr_len += iResult;
            if (curr_len == message_len) {
                curr_len = 0;
                message_len = -1;
                // Handle message
